...
void handle_dns_request(struct Trie* trie, struct Cache* cache, SOCKET sock, struct sockaddr_in clientAddr, const char* remoteDnsAddr)
{
    ...
    while (1)
    {
        ...
        else if (msg->header->qr == 1) // 处理从远程DNS服务器返回的DNS响应报文
        {
            ...
            if (currentid == preId) {
                insertCacheEntry(cache, domain, ipAddr, type, ttl);
            }
            buf[0] = preId >> 8;
            buf[1] = preId;
        ...
        purgeExpiredEntries(cache); // 每次处理完一个DNS请求,删除过期的缓存记录
...
unsigned char* findIpAddress(struct Trie* trie, struct Cache* cache, unsigned char domain[MAX_DOMAIN_LENGTH])
{
    ...
    // 先在缓存表中查找,找到返回
    // 尝试在缓存表中查找IPv4地址
    if (retrieveCacheEntry(cache, domain, ipAddr, 1))
    {
        // 如果找到IPv4地址，打印找到的信息
    ...
    // 尝试在缓存表中查找IPv6地址
    else if (retrieveCacheEntry(cache, domain, ipAddr, 28))
    {
        // 如果找到IPv6地址，打印找到的信息
    ...
    else
    {
        ...
        int node = searchDomainNode(trie, domain);
        ...
        if (node != 0)
        {
            ...
            insertCacheEntry(cache, domain, ipAddr, 1, CACHE_TTL);
            // 打印找到的IP地址信息
...
{
    unsigned char* bytestream = dtob(msg);
    // 检查转换是否成功
    if (bytestream == NULL) {
    ...
    Dns_Msg* temp = btod(bytestream, (unsigned short*)(&len));
    // 检查报文转换是否成功
    if (temp == NULL) {
